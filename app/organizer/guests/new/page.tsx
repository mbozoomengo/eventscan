...
export default function NewGuestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, addAnother = false) => {
    ...
    if (addAnother) { setForm(emptyForm) } else { router.push('/organizer/guests') }
    setLoading(false)
  }
      ...
      <div className="card p-6 max-w-lg">
        <form onSubmit={e => handleSubmit(e, false)} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Link href="/organizer/guests" className="flex-1 text-center btn-secondary text-sm py-2">Annuler</Link>
            <button type="button" disabled={loading} onClick={e => handleSubmit(e as unknown as React.FormEvent, true)}
              className="flex-1 border border-blue-400 text-blue-600 text-sm font-medium py-2 rounded-lg hover:bg-blue-50 disabled:opacity-50 transition-colors">
              + Ajouter un autre
            </button>
