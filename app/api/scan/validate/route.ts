...
export async function POST(request: NextRequest) {
  ...
  if (profile?.role === 'scanner') {
    const { data: teamEntry } = await supabase
      .from('event_team').select('is_blocked')
  ...
  const { data: guest } = await supabase
    .from('guests').select('id, full_name, category, table_name, checked_in, checked_in_at, event_id')
    .eq('qr_token', qr_token).single()
  ...
  if (guest.checked_in) {
    await supabase.from('scan_logs').insert({ guest_id: guest.id, event_id, status: 'already_scanned', scanned_by: user.id })
    return NextResponse.json({ status: 'already_scanned', guest, first_scan_at: guest.checked_in_at })
  }

  await supabase.from('guests').update({ checked_in: true, checked_in_at: new Date().toISOString() }).eq('id', guest.id)
  await supabase.from('scan_logs').insert({ guest_id: guest.id, event_id, status: 'success', scanned_by: user.id })
